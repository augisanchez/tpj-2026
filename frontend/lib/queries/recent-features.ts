...
const RecentFeaturesQuery = gql`
  query RecentFeatures($first: Int = 6) {
    features(
      first: $first
...
type Response = { features: { nodes: Raw[] } };
...
export async function fetchRecentFeatures(first = 6): Promise<RecentEssay[]> {
  const data = await wpClient.request<Response>(RecentFeaturesQuery, {
    first,
  });
  return data.features.nodes.map((node): RecentEssay => {
